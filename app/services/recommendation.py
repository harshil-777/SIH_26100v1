...
def generate_recommendation(overall_score: Decimal | float, risk_level: str, breakdown: dict) -> str:
    ...
    if reasons:
        joined = " ".join(reasons)
        return (
            f"{ADVISORY_PREFIX}This bid failed {len(reasons)} mandatory criterion/criteria "
            f"({joined}) Recommend disqualification, subject to officer review."
        )
    ...
    weak_note = f" Weakest area: {weakest['id']}"
